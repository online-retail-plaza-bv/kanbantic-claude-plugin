...
function loadProxyInSandbox() {
  ...
  // The proxy guards its runtime side-effects — stdin wiring and the
  // SIGINT/SIGTERM registration — behind `require.main === module` (KBT-F464),
  // so that require()'ing it for helper-level unit tests installs nothing.
  // This sandbox is the opposite case: we WANT those handlers registered, and
  // a bare `.bind()` drops the `main` property, leaving the guard false and
  // `listeners` empty. Point require.main at the sandbox module so the proxy
  // sees itself as the entry point. Safe: nothing else hangs off that guard —
  // autoRegister fires from postProcess on an initialize-response, never at
  // load, and fakeProcess.stdin's on() is a no-op.
  const sandboxRequire = proxyModule.require.bind(proxyModule);
  sandboxRequire.main = proxyModule;
  sandboxRequire.resolve = (request) => Module._resolveFilename(request, proxyModule);
  sandboxRequire.cache = require.cache;
