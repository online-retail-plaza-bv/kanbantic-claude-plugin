...
// ---------------------------------------------------------------------------
// KBT-F722 / KBT-SR624 — claudeCliSessionId is attached to every register call
// that is actually forwarded, mirroring attachProcessTokenToRegisterCall exactly
// (same clobber-guard, same no-op-for-non-register-tools contract).
// ---------------------------------------------------------------------------

test('KBT-F722-1 — attachClaudeCliSessionIdToRegisterCall adds CLAUDE_CODE_SESSION_ID when set', () => {
  proxy.__resetForTest();
  process.env.CLAUDE_CODE_SESSION_ID = 'cli-abc';
  try {
    const msg = toolCallMsg('register_agent_session', { workspaceId: 'ws-1' });
    const mutated = proxy.attachClaudeCliSessionIdToRegisterCall(msg);
    assert.strictEqual(mutated, true);
    assert.strictEqual(msg.params.arguments.claudeCliSessionId, 'cli-abc');
  } finally {
...
test('KBT-F722-2 — attachClaudeCliSessionIdToRegisterCall is a no-op when the env var is unset', () => {
  proxy.__resetForTest();
  delete process.env.CLAUDE_CODE_SESSION_ID;
  const msg = toolCallMsg('register_agent_session', { workspaceId: 'ws-1' });
  const mutated = proxy.attachClaudeCliSessionIdToRegisterCall(msg);
  assert.strictEqual(mutated, false);
  assert.strictEqual(msg.params.arguments.claudeCliSessionId, undefined);
  proxy.__resetForTest();
});

test('KBT-F722-3 — attachClaudeCliSessionIdToRegisterCall never clobbers a caller-supplied value', () => {
  proxy.__resetForTest();
  process.env.CLAUDE_CODE_SESSION_ID = 'cli-real';
  try {
    const msg = toolCallMsg('register_agent_session', { workspaceId: 'ws-1', claudeCliSessionId: 'caller-supplied' });
    const mutated = proxy.attachClaudeCliSessionIdToRegisterCall(msg);
    assert.strictEqual(mutated, false, 'must not report a mutation when nothing changed');
    assert.strictEqual(msg.params.arguments.claudeCliSessionId, 'caller-supplied');
  } finally {
    delete process.env.CLAUDE_CODE_SESSION_ID;
    proxy.__resetForTest();
  }
...
test('KBT-F722-4 — attachClaudeCliSessionIdToRegisterCall is a no-op for non-register tools', () => {
  proxy.__resetForTest();
  process.env.CLAUDE_CODE_SESSION_ID = 'cli-abc';
  try {
    const msg = toolCallMsg('heartbeat', { sessionId: 'sess-1' });
...
test('KBT-SR622-1 — sessionFilePath is keyed by CLAUDE_CODE_SESSION_ID, write/read/remove round-trips', () => {
  ...
  try {
    ...
    // KBT-F722 — the CLI session id is now written into the body explicitly, not only
    // implicit via the filename.
    assert.strictEqual(written.claudeCliSessionId, 'test-session-xyz');
