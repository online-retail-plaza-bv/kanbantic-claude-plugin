---
name: kanbantic-specialist-security
description: "Run the Security Specialist (SPEC003) against a Kanbantic workspace, release, issue, or application. Opens a specialist run, delegates the analysis to the security-specialist subagent, records each finding, computes a deterministic health score, and completes the run at status New for human review. Does not auto-review or auto-convert findings."
user_invocable: true
command: specialist-security
---

# Kanbantic Specialist Run — Security

Thin wrapper around the shared specialist-run lifecycle. It runs the **Security Specialist**.

**Announce at start:** "I'm using the kanbantic-specialist-security skill to run the Security Specialist."

## Identity (inputs to the shared core)

| Variable | Value |
|---|---|
| `SPECIALIST_CODE` | `SPEC003` |
| `SPECIALIST_NAME` | `Security Specialist` |
